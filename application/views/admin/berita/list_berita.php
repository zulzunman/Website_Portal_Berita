        ...
        <div class="row py-4">
            ...
            <div class="col">
                <div class="row py-2">
                    ...
                    <div class="col-xl-9">
                        <h4>Data Berita 
                        <?php if( $type=$this->session->userdata('tipe')!='Admin' ) { ?><span class="float-end"><a href="<?=site_url('admin/addb')?>"><button class="btn btn-primary">Tambah Berita</button></a></span>
                        <?php } ?></h4>
                        <hr>
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody class="table-group-divider">
                                    ...
                                    <tr>
                                        ...
                                        <?php if( $type=$this->session->userdata('tipe')!='Admin' ) { ?>
                                        <td align="center"><a href="<?=site_url('admin/updateb/'.$data->id)?>"><i class="fa-solid fa-pen-to-square bg-warning p-2 text-white rounded"></i></a></td>
                                        <?php } ?>
                                        <?php if( $type=$this->session->userdata('tipe')!='Author' ) { ?>
                                            <?php 
                                            if ($data->popular == '0') {
                                                echo '<td align="center"><a href="'.site_url('admin/popular/'.$data->id).'"><button class="btn bg-primary">Popular ON</button></a></td>';
                                            } else {
                                                echo '<td align="center"><a href="'.site_url('admin/gpopular/'.$data->id).'"><button class="btn bg-danger">Popular OFF</button></a></td>';
                                            }
                                            ?>
                                            <?php 
                                            if ($data->utama == '0') {
                                                echo '<td align="center"><a href="'.site_url('admin/utama/'.$data->id).'"><button class="btn bg-primary">Utama ON</button></a></td>';
                                            } else {
                                                echo '<td align="center"><a href="'.site_url('admin/gutama/'.$data->id).'"><button class="btn bg-danger">Utama OFF</button></a></td>';
                                            }
                                            ?>
                                        <td align="center"><a href="<?=site_url('admin/delb/'.$data->id)?>"onclick="return confirm('Are you sure?')"><i class="fa-solid fa-trash bg-danger p-2 text-white rounded"></i></a></td>
                                        <?php } ?>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                        ...
                        <a href="<?=site_url('admin')?>"><input type="button" class="btn btn-primary" value="Back"></a>
                    </div>
                    <div class="col-md-1"></div>
                </div>
            </div>
